...
fn main() -> Result<(), std::io::Error> {
    ...
    for i in 0..instructions.len() {
        if let Some(uncorrupted) = uncorrupt(&instructions, i) {
...
fn uncorrupt(corrupted_instructions: &[Instruction], index: usize) -> Option<Vec<Instruction>> {
    match corrupted_instructions[index] {
        Instruction::Nop(_) | Instruction::Jmp(_) => {
            let mut fixed = corrupted_instructions.to_vec();
