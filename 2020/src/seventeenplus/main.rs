...
fn main() -> Result<(), std::io::Error> {
    let file = File::open("src/seventeenplus/input")?;
...
impl ConwayCubes {
    ...
    fn neighbouring_states<'a>(&'a self, coord: Coord) -> impl Iterator<Item = CubeState> + 'a {
        ((coord.x - 1)..=(coord.x + 1))
            .cartesian_product((coord.y - 1)..=(coord.y + 1))
            .cartesian_product((coord.z - 1)..=(coord.z + 1))
            .cartesian_product((coord.w - 1)..=(coord.w + 1))
            .filter_map(move |(((x, y), z), w)| {
                let candidate = Coord { x, y, z, w };
                if candidate != coord {
    ...
    fn next_coords(&self) -> impl Iterator<Item = Coord> {
        ...
        (next_min.x..=next_max.x)
            .cartesian_product(next_min.y..=next_max.y)
            .cartesian_product(next_min.z..=next_max.z)
            .cartesian_product(next_min.w..=next_max.w)
            .map(|(((x, y), z), w)| Coord { x, y, z, w })
    }
